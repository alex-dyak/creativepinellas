...
class ls_sub_pages extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'ls_sub_pages', // Base ID
			__( 'Sub Pages', 'subpage' ), // Name
			array( 'description' => __( 'Displays the list of subpages for that particular parent page else will display the list of parent pages.', 'subpage' ), ) // Args
		);
	...
	public function form( $instance ) {
		...
		<p><label
				for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'subpage' ); ?>
				<input class="widefat"
		...
		<label
			for="help_text"><?php _e( 'Options for Displaying the Subpages', 'subpage' ) ?></label>
		<p>
			<label
				for="<?php echo $this->get_field_id( 'sort_order' ); ?>"><?php _e( 'Sorting Order', 'subpage' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'sort_order' ); ?>"
				...
				foreach ( $order_options as $option_key => $option_value ) {
					?>
					<option <?php selected( $instance['sort_order'], $option_key ); ?>
						value="<?php echo esc_attr( $option_key ); ?>"><?php echo __( $option_value, 'subpage' ); ?></option>
				<?php } ?>
			</select>&nbsp;&nbsp;
		</p>
		...
		<label
			for="help_text1"><?php _e( 'Options for Displaying the Parent Pages when their is no subpages to be displayed', 'subpage' ); ?></label>
		<p>
			<label
				for="<?php echo $this->get_field_id( 'sort_by' ); ?>"><?php _e( 'Sorting Criteria', 'subpage' ); ?></label>
			<select multiple="multiple"
			        name="<?php echo $this->get_field_name( 'sort_by' ); ?>[]"
			        id="<?php echo $this->get_field_id( 'sort_by' ); ?>"
			        class="widefat">
				...
				foreach ( $options as $key => $value ) {
					...
					<option <?php echo $selected ?>
						value="<?php echo esc_attr( $key ); ?>"><?php echo __( $value, 'subpage' ); ?></option>
				<?php } ?>
			</select>
		</p>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'exclude_pages' ); ?>"><?php _e( 'Exclude Pages', 'subpage' ); ?></label>

			<select multiple="multiple"
			        name="<?php echo $this->get_field_name( 'exclude_pages' ); ?>[]"
			        id="<?php echo $this->get_field_id( 'exclude_pages' ); ?>"
			        class="widefat">
				...
				foreach ( $pages as $page ) {
					...
					?>
					<option <?php echo $selected; ?>
						value="<?php echo esc_attr( $page->ID ); ?>"><?php echo $page->post_title; ?></option>
				<?php
				}
				?>
			</select>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'depth' ); ?>"><?php _e( 'Depth Level', 'subpage' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'depth' ); ?>"
				...
				foreach ( $depth_options as $depth_number => $depth_label ) {
					?>
					<option <?php selected( $instance['depth'], $depth_number ); ?>
						value="<?php echo esc_attr( $depth_number ); ?>"><?php echo __( $depth_label, 'subpage' ); ?></option>
				<?php } ?>
			</select>&nbsp;&nbsp;
		</p>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'sort_order' ); ?>"><?php _e( 'Sorting Order For Parent 	Pages', 'subpage' ); ?></label>
			<select
				name="<?php echo $this->get_field_name( 'sort_order_parent' ); ?>"
				id="<?php echo $this->get_field_id( 'sort_order_parent' ); ?>"
				class="widefat">
				<?php
				$order_options = array(
					'ASC'  => __( 'Ascending', 'subpage' ),
				...
				foreach ( $order_options as $option_key => $option_value ) {
					?>
					<option <?php selected( $instance['sort_order_parent'], $option_key ); ?>
						value="<?php echo esc_attr( $option_key ); ?>"><?php echo __( $option_value, 'subpage' ); ?></option>
				<?php } ?>
			</select>&nbsp;&nbsp;
		</p>
	...
	function widget( $args, $instance ) {
		...
		if ( $attachments ) {
			...
			<ul class="ls_page_list" style="padding-left: 0; margin-left: 0;">
				<?php
				foreach ( $attachments as $attachment ) {
					?>
					<li style="list-style-type: none;">
						<a href="<?php echo the_permalink( $attachment->ID ); ?>"><?php echo $attachment->post_title; ?></a>
					</li>
				<?php
				}?>
			</ul>
