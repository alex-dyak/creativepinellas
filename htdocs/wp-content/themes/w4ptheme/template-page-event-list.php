		...
		<?php if ( get_field( 'event_list_link_page' ) ) : ?>
			<section class="row column">
				<p class="u-text--center">
					<a href="<?php the_field( 'event_list_link_page' ); ?>"
					   class="btn btn-middleWidth"><?php echo __( 'SUBMIT YOUR EVENT',
							'w4ptheme' ); ?></a>
				</p>
			</section>
		<?php endif; ?>
...
<section class="u-background--gray directoryFilters">
	<section class="row">
		<div class="column">
			<h2 class="u-text--center"><?php echo __( 'EASY SEARCH',
					'w4ptheme' ); ?></h2>
	...
	<form action="<?php the_permalink() ?>"
		...
		<section class="row">
			<div class="halfMedium-6 medium-4 column">
				<label for="attend"><?php echo __( 'Who Should Attend',
						'w4ptheme' ); ?></label>
			...
			<div class="halfMedium-6 medium-4 column">
				<label for="eventType"><?php echo __( 'Type of Event',
						'w4ptheme' ); ?></label>
		...
		<section class="row">
			<div class="halfMedium-6 medium-3 medium-offset-1 column">
				<label for="dateFrom"><?php echo __( 'Date Range: From',
						'w4ptheme' ); ?></label>
				...
				<div class="form-row">
					<input type="text" name="dateFrom" id="dateFrom"
					       ...
					       value="<?php echo ! empty( $_GET['dateFrom'] )
						       ? $_GET['dateFrom'] : null; ?>">
				</div>
			</div>
			<div class="halfMedium-6 medium-3 column">
				<label for="dateTo"><?php echo __( 'To',
						'w4ptheme' ); ?></label>
				...
				<div class="form-row">
					<input type="text" name="dateTo" id="dateTo"
					       placeholder="all"
					       class="is-datepicker js-datepicker"
					       value="<?php echo ! empty( $_GET['dateTo'] )
						       ? $_GET['dateTo'] : null; ?>">
				</div>
			</div>
							...
							<a href="<?php the_permalink(); ?>"
							   class="js-touchFocus">
								<?php if ( has_post_thumbnail() ) : ?>
									<img
										src="<?php echo get_the_post_thumbnail_url( get_the_ID(), 'big_blog_img' ); ?>" alt=""
										srcset="<?php echo get_the_post_thumbnail_url( get_the_ID(),
											'big_blog_img' ); ?> 768w, <?php echo get_the_post_thumbnail_url( get_the_ID(),
											'img_680x680' ); ?> 1024w,  <?php echo get_the_post_thumbnail_url( get_the_ID(),
											'img_940x940' ); ?> 1400w">
									<?php else : ?>
								...
								<div class="gridItem-info">
									...
									<?php $EM_Event
										= em_get_event( get_the_ID(),
										'post_id' ); ?>
									<?php $start_date
										= $EM_Event->event_start_date
										? date_create( $EM_Event->event_start_date )
										: $start_date = null; ?>
									<?php $end_date = $EM_Event->event_end_date
										? date_create( $EM_Event->event_end_date )
										: $end_date = null; ?>
									<p class="u-text--upper">
										<i>
											<?php if ( $EM_Event->event_start_date
											           == $EM_Event->event_end_date
											) : ?>
												<?php echo date_format( $start_date,
													'M. j' ); ?>
											<?php else : ?>
												<?php echo
